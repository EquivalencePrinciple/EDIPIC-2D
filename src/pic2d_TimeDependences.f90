...
SUBROUTINE INITIATE_PROBE_DIAGNOSTICS
  ...
  CHARACTER(38) clusterprobext_filename    ! _probelocs_cluster_NNNNN_NNNNN_ext.dat
                                           ! ----x----I----x----I----x----I----x---
...
! save probe positions
     ...
     DO n = 1, N_of_probes
        WRITE (20, '(2x,i3,2(2x,i5),2(2x,e14.7))') &
             ...
             & Probe_position(1,n), &
             & Probe_position(2,n), &
             & Probe_position(1,n) * delta_x_m * 1000.0, &
             & Probe_position(2,n) * delta_x_m * 1000.0
     END DO
     CLOSE (20, STATUS = 'KEEP')
     PRINT '("### file _probelocs.dat is ready ###")'

! save positions of probes for EX (shifted by half-a-cell in the x-direction
     OPEN (20, FILE = '_probelocs_EX.dat')
     WRITE (20, '("# col  1 is the global probe number")')
     WRITE (20, '("# col  2 is index i of probe position in computational grid (along x)")')
     WRITE (20, '("# col  3 is index j of probe position in computational grid (along y)")')
     WRITE (20, '("# col  4 is probe cartesian coordinate x [mm]")')
     ...
     DO n = 1, N_of_probes
        WRITE (20, '(2x,i3,2(2x,i5),2(2x,e14.7))') &
             ...
             & Probe_position(1,n), &
             & Probe_position(2,n), &
             & (REAL(Probe_position(1,n)) + 0.5) * delta_x_m * 1000.0, &
             & Probe_position(2,n) * delta_x_m * 1000.0
     END DO
     CLOSE (20, STATUS = 'KEEP')
     PRINT '("### file _probelocs_EX.dat is ready ###")'

! save positions of probes for EY (shifted by half-a-cell in the y-direction
     OPEN (20, FILE = '_probelocs_EY.dat')
     WRITE (20, '("# col  1 is the global probe number")')
     WRITE (20, '("# col  2 is index i of probe position in computational grid (along x)")')
     WRITE (20, '("# col  3 is index j of probe position in computational grid (along y)")')
     WRITE (20, '("# col  4 is probe cartesian coordinate x [mm]")')
     ...
     DO n = 1, N_of_probes
        WRITE (20, '(2x,i3,2(2x,i5),2(2x,e14.7))') &
             ...
             & Probe_position(2,n), &
             & Probe_position(1,n) * delta_x_m * 1000.0, &
             & (REAL(Probe_position(2,n)) + 0.5) * delta_x_m * 1000.0
     END DO
     CLOSE (20, STATUS = 'KEEP')
     PRINT '("### file _probelocs_EY.dat is ready ###")'

  END IF
        
! master processes will keep their own probes only
! note, N_of_probes_cluster = 0 and N_of_probes_cluster_ext = 0 are in INITIATE_PARAMETERS to ensure correct call of DISTRIBUTE_CLUSTER_PARAMETERS 
...
! now we use the extended domain, similar to the one used for calculation of VDF moments
     DO npa = 1, N_of_probes
        ...
        END IF
        IF ( (Probe_position(1,npa).GE.c_indx_x_min_ext).AND. &
           & (Probe_position(1,npa).LE.c_indx_x_max_ext).AND. &
           & (Probe_position(2,npa).GE.c_indx_y_min_ext).AND. &
           & (Probe_position(2,npa).LE.c_indx_y_max_ext) ) THEN
           N_of_probes_cluster_ext = N_of_probes_cluster_ext + 1
        END IF
     END DO

     PRINT '("Cluster ",i5," : has ",i3," probes in the ordinary domain and ",i3," probes in the extended domain")', &
          & Rank_of_process, N_of_probes_cluster, N_of_probes_cluster_ext
...
!                                 ----x----I----x----I----x----I----
     ...
     IF (N_of_probes_cluster_ext.GT.0) THEN
        ...
        ALLOCATE(List_of_probes_cluster_ext(1:N_of_probes_cluster_ext), STAT = ALLOC_ERR)
        npc = 0
        DO npa = 1, N_of_probes
           IF ( (Probe_position(1,npa).GE.c_indx_x_min_ext).AND. &
              & (Probe_position(1,npa).LE.c_indx_x_max_ext).AND. &
              ...
              & (Probe_position(2,npa).LE.c_indx_y_max_ext) ) THEN
              npc = npc+1
              List_of_probes_cluster_ext(npc) = npa            
           END IF
        END DO

        ALLOCATE(probe_Ne_cluster(1:N_of_probes_cluster_ext), STAT = ALLOC_ERR)

        ALLOCATE(probe_VXe_cluster(1:N_of_probes_cluster_ext), STAT = ALLOC_ERR)
        ALLOCATE(probe_VYe_cluster(1:N_of_probes_cluster_ext), STAT = ALLOC_ERR)
        ALLOCATE(probe_VZe_cluster(1:N_of_probes_cluster_ext), STAT = ALLOC_ERR)

        ALLOCATE(probe_WXe_cluster(1:N_of_probes_cluster_ext), STAT = ALLOC_ERR)
        ALLOCATE(probe_WYe_cluster(1:N_of_probes_cluster_ext), STAT = ALLOC_ERR)
        ALLOCATE(probe_WZe_cluster(1:N_of_probes_cluster_ext), STAT = ALLOC_ERR)

        ALLOCATE(probe_VXVYe_cluster(1:N_of_probes_cluster_ext), STAT = ALLOC_ERR)
        ALLOCATE(probe_VXVZe_cluster(1:N_of_probes_cluster_ext), STAT = ALLOC_ERR)
        ALLOCATE(probe_VYVZe_cluster(1:N_of_probes_cluster_ext), STAT = ALLOC_ERR)

        ALLOCATE(probe_QXe_cluster(1:N_of_probes_cluster_ext), STAT = ALLOC_ERR)
        ALLOCATE(probe_QYe_cluster(1:N_of_probes_cluster_ext), STAT = ALLOC_ERR)
        ALLOCATE(probe_QZe_cluster(1:N_of_probes_cluster_ext), STAT = ALLOC_ERR)

        ALLOCATE(probe_Ni_cluster(1:N_of_probes_cluster_ext,1:N_spec), STAT = ALLOC_ERR)

        ALLOCATE(probe_VXi_cluster(1:N_of_probes_cluster_ext,1:N_spec), STAT = ALLOC_ERR)
        ALLOCATE(probe_VYi_cluster(1:N_of_probes_cluster_ext,1:N_spec), STAT = ALLOC_ERR)
        ALLOCATE(probe_VZi_cluster(1:N_of_probes_cluster_ext,1:N_spec), STAT = ALLOC_ERR)

        ALLOCATE(probe_WXi_cluster(1:N_of_probes_cluster_ext,1:N_spec), STAT = ALLOC_ERR)
        ALLOCATE(probe_WYi_cluster(1:N_of_probes_cluster_ext,1:N_spec), STAT = ALLOC_ERR)
        ALLOCATE(probe_WZi_cluster(1:N_of_probes_cluster_ext,1:N_spec), STAT = ALLOC_ERR)

        ALLOCATE(probe_VXVYi_cluster(1:N_of_probes_cluster_ext,1:N_spec), STAT = ALLOC_ERR)
        ALLOCATE(probe_VXVZi_cluster(1:N_of_probes_cluster_ext,1:N_spec), STAT = ALLOC_ERR)
        ALLOCATE(probe_VYVZi_cluster(1:N_of_probes_cluster_ext,1:N_spec), STAT = ALLOC_ERR)

        ALLOCATE(probe_QXi_cluster(1:N_of_probes_cluster_ext,1:N_spec), STAT = ALLOC_ERR)
        ALLOCATE(probe_QYi_cluster(1:N_of_probes_cluster_ext,1:N_spec), STAT = ALLOC_ERR)
        ALLOCATE(probe_QZi_cluster(1:N_of_probes_cluster_ext,1:N_spec), STAT = ALLOC_ERR)

! save probe positions
        clusterprobext_filename = '_probelocs_cluster_NNNNN_NNNNN_ext.dat'
!                                  ----x----I----x----I----x----I----x---
        clusterprobext_filename(20:24) = convert_int_to_txt_string(Rank_of_process, 5)
        clusterprobext_filename(26:30) = convert_int_to_txt_string(Rank_horizontal, 5)

        OPEN (20, FILE = clusterprobext_filename)
        WRITE (20, '("# col  1 is the global probe number")')
        WRITE (20, '("# col  2 is index i of probe position in computational grid (along x)")')
        WRITE (20, '("# col  3 is index j of probe position in computational grid (along y)")')
        WRITE (20, '("# col  4 is probe cartesian coordinate x [mm]")')
        ...
        END DO
        CLOSE (20, STATUS = 'KEEP')
        PRINT '("Process ",i5," :: file ",A38," is ready")', Rank_of_process, clusterprobext_filename

     END IF   !###   IF (N_of_probes_cluster_ext.GT.0) THEN
...
! particle calculators are already assigned, so we must tell them about the probes in the master process
     ...
     CALL MPI_BCAST(N_of_probes_cluster_ext, 1, MPI_INTEGER, 0, COMM_CLUSTER, ierr)

     IF (N_of_probes_cluster_ext.GT.0) THEN
        CALL MPI_BCAST(List_of_probes_cluster_ext, N_of_probes_cluster_ext, MPI_INTEGER, 0, COMM_CLUSTER, ierr)        
     END IF
...
! only cluster masters left
! now all clusters must tell process 0 about the probes they have 

  IF (Rank_horizontal.GT.0) THEN 

     ALLOCATE(ibufer(MAX(2,N_of_probes_cluster+N_of_probes_cluster_ext)), STAT = ALLOC_ERR)

     ibufer(1) = N_of_probes_cluster
     ibufer(2) = N_of_probes_cluster_ext
! send number of probes to the master with zero rank     
     CALL MPI_SEND(ibufer(1:2), 2, MPI_INTEGER, 0, Rank_horizontal, COMM_HORIZONTAL, request, ierr) 
        
! send global indices of probes to the master with zero rank
     bufsize = N_of_probes_cluster + N_of_probes_cluster_ext
     IF (bufsize.GT.0) THEN
        DO i = 1, N_of_probes_cluster
           ibufer(i) = List_of_probes_cluster(i)
        END DO
        DO i = 1, N_of_probes_cluster_ext
           ibufer(i+N_of_probes_cluster) = List_of_probes_cluster_ext(i)
        END DO
        CALL MPI_SEND(ibufer, bufsize, MPI_INTEGER, 0, Rank_horizontal+100000, COMM_HORIZONTAL, request, ierr) 
     END IF

     DEALLOCATE(ibufer, STAT = ALLOC_ERR)
! masters with non-zero rank leave here
     RETURN

  END IF   !###   IF (Rank_horizontal.EQ.0) THEN 

! the rest is performed by the cluster master with COMM_HORIZONTAL rank zero

  ALLOCATE(ibufer(MAX(2,N_of_probes+N_of_probes)), STAT = ALLOC_ERR)   ! this size should be enough
...
! receive number of probes from each cluster
  DO n = 1, N_processes_horizontal-1
     CALL MPI_RECV(ibufer(1:2), 2, MPI_INTEGER, n, n, COMM_HORIZONTAL, stattus, ierr)
     diag_cluster(n)%N_probes = ibufer(1)
     diag_cluster(n)%N_probes_ext = ibufer(2)
     IF (diag_cluster(n)%N_probes.GT.0)     ALLOCATE(diag_cluster(n)%probe_number(1:diag_cluster(n)%N_probes), STAT=ALLOC_ERR)
     IF (diag_cluster(n)%N_probes_ext.GT.0) ALLOCATE(diag_cluster(n)%probe_number_ext(1:diag_cluster(n)%N_probes_ext), STAT=ALLOC_ERR)
  END DO

! receive sets of indices [in the global array of probes] of probes from each cluster
  DO n = 1, N_processes_horizontal-1
     bufsize = diag_cluster(n)%N_probes + diag_cluster(n)%N_probes_ext
     IF (bufsize.LE.0) CYCLE
     CALL MPI_RECV(ibufer(1:bufsize), bufsize, MPI_INTEGER, n, n+100000, COMM_HORIZONTAL, stattus, ierr)
     IF (diag_cluster(n)%N_probes.GT.0)     diag_cluster(n)%probe_number(1:diag_cluster(n)%N_probes) = ibufer(1:diag_cluster(n)%N_probes)
     IF (diag_cluster(n)%N_probes_ext.GT.0) diag_cluster(n)%probe_number_ext(1:diag_cluster(n)%N_probes_ext) = ibufer(diag_cluster(n)%N_probes+1:diag_cluster(n)%N_probes+diag_cluster(n)%N_probes_ext)
  END DO
...
! ion heat flow QZ
     ...
     END DO   !###   DO s = 1, N_spec
  END IF   !###   IF (use_checkpoint.EQ.1) THEN

END SUBROUTINE INITIATE_PROBE_DIAGNOSTICS

!------------------------------
!
SUBROUTINE SAVE_PROBES_DATA
...
! assemble probe data from all other clusters
     DO n = 1, N_processes_horizontal-1
        ...
        N_probes = diag_cluster(n)%N_probes
        N_probes_ext = diag_cluster(n)%N_probes_ext
! 3 x field :: F,EX,EY
!13 x vdf moments e/i :: N,VX,VY,VZ,WX,WY,WZ,VXVY,VXVZ,VYVZ,QX,QY,QZ
        bufsize = 3 * N_probes + 13 * (1 + N_spec) * N_probes_ext

        IF (bufsize.EQ.0) CYCLE

        ALLOCATE(rbufer(1:bufsize), STAT = ALLOC_ERR)

        CALL MPI_RECV(rbufer(1:bufsize), bufsize, MPI_REAL, n, n, COMM_HORIZONTAL, stattus, ierr)

! if a probe is at a boundary between clusters, it is included in both clusters
! for such probes, data like EX, EY, and F are simply overwritten
        DO npc = 1, N_probes
           npa = diag_cluster(n)%probe_number(npc)
           probe_F(npa) = rbufer(npc)
           probe_EX(npa) = rbufer(npc+N_probes)
           probe_EY(npa) = rbufer(npc+N_probes+N_probes)
        END DO   !###  DO npc = 1, recsize 
           
! while densities, currents, and integral energies below are accumulated
        shift = 3 * N_probes
        DO npc = 1, N_probes_ext
           npa = diag_cluster(n)%probe_number_ext(npc)

           pos = npc+shift
           probe_Ne(npa) = probe_Ne(npa) + rbufer(pos) 

           pos = pos+N_probes_ext
           probe_VXe(npa) = probe_VXe(npa) + rbufer(pos)
           pos = pos+N_probes_ext
           probe_VYe(npa) = probe_VYe(npa) + rbufer(pos)
           pos = pos+N_probes_ext
           probe_VZe(npa) = probe_VZe(npa) + rbufer(pos)

           pos = pos+N_probes_ext
           probe_WXe(npa) = probe_WXe(npa) + rbufer(pos)
           pos = pos+N_probes_ext
           probe_WYe(npa) = probe_WYe(npa) + rbufer(pos)
           pos = pos+N_probes_ext
           probe_WZe(npa) = probe_WZe(npa) + rbufer(pos)

           pos = pos+N_probes_ext
           probe_VXVYe(npa) = probe_VXVYe(npa) + rbufer(pos)
           pos = pos+N_probes_ext
           probe_VXVZe(npa) = probe_VXVZe(npa) + rbufer(pos)
           pos = pos+N_probes_ext
           probe_VYVZe(npa) = probe_VYVZe(npa) + rbufer(pos)

           pos = pos+N_probes_ext
           probe_QXe(npa) = probe_QXe(npa) + rbufer(pos)
           pos = pos+N_probes_ext
           probe_QYe(npa) = probe_QYe(npa) + rbufer(pos)
           pos = pos+N_probes_ext
           probe_QZe(npa) = probe_QZe(npa) + rbufer(pos)

           DO s = 1, N_spec
              pos = pos+N_probes_ext
              probe_Ni(npa, s) = probe_Ni(npa, s) + rbufer(pos) 

              pos = pos+N_probes_ext
              probe_VXi(npa, s) = probe_VXi(npa, s) + rbufer(pos)
              pos = pos+N_probes_ext
              probe_VYi(npa, s) = probe_VYi(npa, s) + rbufer(pos)
              pos = pos+N_probes_ext
              probe_VZi(npa, s) = probe_VZi(npa, s) + rbufer(pos)

              pos = pos+N_probes_ext
              probe_WXi(npa, s) = probe_WXi(npa, s) + rbufer(pos)
              pos = pos+N_probes_ext
              probe_WYi(npa, s) = probe_WYi(npa, s) + rbufer(pos)
              pos = pos+N_probes_ext
              probe_WZi(npa, s) = probe_WZi(npa, s) + rbufer(pos)

              pos = pos+N_probes_ext
              probe_VXVYi(npa, s) = probe_VXVYi(npa, s) + rbufer(pos)
              pos = pos+N_probes_ext
              probe_VXVZi(npa, s) = probe_VXVZi(npa, s) + rbufer(pos)
              pos = pos+N_probes_ext
              probe_VYVZi(npa, s) = probe_VYVZi(npa, s) + rbufer(pos)

              pos = pos+N_probes_ext
              probe_QXi(npa, s) = probe_QXi(npa, s) + rbufer(pos)
              pos = pos+N_probes_ext
              probe_QYi(npa, s) = probe_QYi(npa, s) + rbufer(pos)
              pos = pos+N_probes_ext
              probe_QZi(npa, s) = probe_QZi(npa, s) + rbufer(pos)
           END DO 
        END DO   !###  DO npc = 1, N_probes_ext
...
! calculate derived values
     ...
     DO npa = 1, N_of_probes
        ...
        IF (probe_Ne(npa).GT.1.0e-9) THEN
...
! electron heat flow QXe 
     ...
     WRITE (21, '(2x,f14.6,100(1x,e14.7))') time_ns, probe_QXe
     CLOSE (21, STATUS = 'KEEP')

! electron heat flow QYe 
     OPEN  (21, FILE = 'dim_QYe_vst.dat', POSITION = 'APPEND')
     WRITE (21, '(2x,f14.6,100(1x,e14.7))') time_ns, probe_QYe
     CLOSE (21, STATUS = 'KEEP')

! electron heat flow QZe 
     OPEN  (21, FILE = 'dim_QZe_vst.dat', POSITION = 'APPEND')
     WRITE (21, '(2x,f14.6,100(1x,e14.7))') time_ns, probe_QZe
     CLOSE (21, STATUS = 'KEEP')
...
! average ion heat flow QX
        NNis_filename = 'dim_QXi_S_vst.dat'
        NNis_filename(9:9) = convert_int_to_txt_string(s, 1)
        OPEN  (21, FILE = NNis_filename, POSITION = 'APPEND')
        WRITE (21, '(2x,f14.6,100(1x,e14.7))') time_ns, probe_QXi(1:N_of_probes, s)
        CLOSE (21, STATUS = 'KEEP')

! average ion heat flow QY
        NNis_filename = 'dim_QYi_S_vst.dat'
        NNis_filename(9:9) = convert_int_to_txt_string(s, 1)
        OPEN  (21, FILE = NNis_filename, POSITION = 'APPEND')
        WRITE (21, '(2x,f14.6,100(1x,e14.7))') time_ns, probe_QYi(1:N_of_probes, s)
        CLOSE (21, STATUS = 'KEEP')

! average ion heat flow QZ
        NNis_filename = 'dim_QZi_S_vst.dat'
        NNis_filename(9:9) = convert_int_to_txt_string(s, 1)
        OPEN  (21, FILE = NNis_filename, POSITION = 'APPEND')
        WRITE (21, '(2x,f14.6,100(1x,e14.7))') time_ns, probe_QZi(1:N_of_probes, s)
        CLOSE (21, STATUS = 'KEEP')
  ...
  ELSE   !###   IF (Rank_horizontal.EQ.0) THEN
     ...
     bufsize = 3 * N_of_probes_cluster + 13 * (1 + N_spec) * N_of_probes_cluster_ext
...
! send data to the zero cluster master
     ...
     DO npc = 1, N_of_probes_cluster
        ...
        i = Probe_position(1,npa)
        j = Probe_position(2,npa)
        rbufer(npc)                                         = c_phi_ext(i, j)
        rbufer(npc+N_of_probes_cluster)                     = EX(i, j)
        rbufer(npc+N_of_probes_cluster+N_of_probes_cluster) = EY(i, j)
     END DO

     pos1 = 3 * N_of_probes_cluster + 1
     pos2 = 3 * N_of_probes_cluster + N_of_probes_cluster_ext
     rbufer(pos1:pos2) = probe_Ne_cluster(1:N_of_probes_cluster_ext)

     pos1 = pos1 + N_of_probes_cluster_ext
     pos2 = pos2 + N_of_probes_cluster_ext
     rbufer(pos1:pos2) = probe_VXe_cluster(1:N_of_probes_cluster_ext)

     pos1 = pos1 + N_of_probes_cluster_ext
     pos2 = pos2 + N_of_probes_cluster_ext
     rbufer(pos1:pos2) = probe_VYe_cluster(1:N_of_probes_cluster_ext)

     pos1 = pos1 + N_of_probes_cluster_ext
     pos2 = pos2 + N_of_probes_cluster_ext
     rbufer(pos1:pos2) = probe_VZe_cluster(1:N_of_probes_cluster_ext)

     pos1 = pos1 + N_of_probes_cluster_ext
     pos2 = pos2 + N_of_probes_cluster_ext
     rbufer(pos1:pos2) = probe_WXe_cluster(1:N_of_probes_cluster_ext)

     pos1 = pos1 + N_of_probes_cluster_ext
     pos2 = pos2 + N_of_probes_cluster_ext
     rbufer(pos1:pos2) = probe_WYe_cluster(1:N_of_probes_cluster_ext)

     pos1 = pos1 + N_of_probes_cluster_ext
     pos2 = pos2 + N_of_probes_cluster_ext
     rbufer(pos1:pos2) = probe_WZe_cluster(1:N_of_probes_cluster_ext)

     pos1 = pos1 + N_of_probes_cluster_ext
     pos2 = pos2 + N_of_probes_cluster_ext
     rbufer(pos1:pos2) = probe_VXVYe_cluster(1:N_of_probes_cluster_ext)

     pos1 = pos1 + N_of_probes_cluster_ext
     pos2 = pos2 + N_of_probes_cluster_ext
     rbufer(pos1:pos2) = probe_VXVZe_cluster(1:N_of_probes_cluster_ext)

     pos1 = pos1 + N_of_probes_cluster_ext
     pos2 = pos2 + N_of_probes_cluster_ext
     rbufer(pos1:pos2) = probe_VYVZe_cluster(1:N_of_probes_cluster_ext)

     pos1 = pos1 + N_of_probes_cluster_ext
     pos2 = pos2 + N_of_probes_cluster_ext
     rbufer(pos1:pos2) = probe_QXe_cluster(1:N_of_probes_cluster_ext)

     pos1 = pos1 + N_of_probes_cluster_ext
     pos2 = pos2 + N_of_probes_cluster_ext
     rbufer(pos1:pos2) = probe_QYe_cluster(1:N_of_probes_cluster_ext)

     pos1 = pos1 + N_of_probes_cluster_ext
     pos2 = pos2 + N_of_probes_cluster_ext
     rbufer(pos1:pos2) = probe_QZe_cluster(1:N_of_probes_cluster_ext)

     DO s = 1, N_spec
        pos1 = pos1 + N_of_probes_cluster_ext
        pos2 = pos2 + N_of_probes_cluster_ext
        rbufer(pos1:pos2) = probe_Ni_cluster(1:N_of_probes_cluster_ext, s)

        pos1 = pos1 + N_of_probes_cluster_ext
        pos2 = pos2 + N_of_probes_cluster_ext
        rbufer(pos1:pos2) = probe_VXi_cluster(1:N_of_probes_cluster_ext, s)

        pos1 = pos1 + N_of_probes_cluster_ext
        pos2 = pos2 + N_of_probes_cluster_ext
        rbufer(pos1:pos2) = probe_VYi_cluster(1:N_of_probes_cluster_ext, s)

        pos1 = pos1 + N_of_probes_cluster_ext
        pos2 = pos2 + N_of_probes_cluster_ext
        rbufer(pos1:pos2) = probe_VZi_cluster(1:N_of_probes_cluster_ext, s)

        pos1 = pos1 + N_of_probes_cluster_ext
        pos2 = pos2 + N_of_probes_cluster_ext
        rbufer(pos1:pos2) = probe_WXi_cluster(1:N_of_probes_cluster_ext, s)

        pos1 = pos1 + N_of_probes_cluster_ext
        pos2 = pos2 + N_of_probes_cluster_ext
        rbufer(pos1:pos2) = probe_WYi_cluster(1:N_of_probes_cluster_ext, s)

        pos1 = pos1 + N_of_probes_cluster_ext
        pos2 = pos2 + N_of_probes_cluster_ext
        rbufer(pos1:pos2) = probe_WZi_cluster(1:N_of_probes_cluster_ext, s)

        pos1 = pos1 + N_of_probes_cluster_ext
        pos2 = pos2 + N_of_probes_cluster_ext
        rbufer(pos1:pos2) = probe_VXVYi_cluster(1:N_of_probes_cluster_ext, s)

        pos1 = pos1 + N_of_probes_cluster_ext
        pos2 = pos2 + N_of_probes_cluster_ext
        rbufer(pos1:pos2) = probe_VXVZi_cluster(1:N_of_probes_cluster_ext, s)

        pos1 = pos1 + N_of_probes_cluster_ext
        pos2 = pos2 + N_of_probes_cluster_ext
        rbufer(pos1:pos2) = probe_VYVZi_cluster(1:N_of_probes_cluster_ext, s)

        pos1 = pos1 + N_of_probes_cluster_ext
        pos2 = pos2 + N_of_probes_cluster_ext
        rbufer(pos1:pos2) = probe_QXi_cluster(1:N_of_probes_cluster_ext, s)

        pos1 = pos1 + N_of_probes_cluster_ext
        pos2 = pos2 + N_of_probes_cluster_ext
        rbufer(pos1:pos2) = probe_QYi_cluster(1:N_of_probes_cluster_ext, s)

        pos1 = pos1 + N_of_probes_cluster_ext
        pos2 = pos2 + N_of_probes_cluster_ext
        rbufer(pos1:pos2) = probe_QZi_cluster(1:N_of_probes_cluster_ext, s)
     END DO
...
END SUBROUTINE SAVE_PROBES_DATA
