...
! setup general counters of collision events for minimal diagnostics
     ...
     DO p = 1, collision_e_neutral(n)%N_of_activated_colproc  !neutral(n)%N_en_colproc
        ...
        IF (cluster_rank_key.EQ.0) THEN
           ...
           DO j = c_indx_y_min, c_indx_y_max
              DO i = c_indx_x_min, c_indx_x_max
                 diagnostics_neutral(n)%activated_collision(p)%counter_local(i,j) = &
                      & diagnostics_neutral(n)%activated_collision(p)%counter_local(i,j) + rbufer2(pos)
                 pos=pos+1
              END DO
           END DO
        END IF
