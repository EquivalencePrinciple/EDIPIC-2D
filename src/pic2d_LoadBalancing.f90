...
! receive numbers of particles 
        ...
        DO n = N_processes_cluster-N_processes_to_free_in_cluster, N_processes_cluster-1
           ...
           Length_to_receive(n) = 8 * N_part_to_receive_from_proc(0,n)
           DO s = 1, N_spec !ions
...
! place electrons first
           ...
           DO k = 1, N_electrons
              ...
              rbufer(i+7) = DBLE(electron(k)%tag) 
              i = i + 8
           END DO
! place ions
           DO s = 1, N_spec
...
! calculate total number of particles to be acquired
     ...
     END DO
     Length_to_distribute = 8 * total_N_to_distribute
...
! prepare start positions for each species
     distr_pos(0) = 1
     distr_pos(1) = 1 + 8 * N_to_distribute(0)
     IF(N_spec.ge.2)THEN
        DO s = 2, N_spec
...
! electrons
     IF (N_electrons.GT.avg_N_particles(0)) THEN
        DO k = avg_N_particles(0)+1, N_electrons
           ...
           rbufer(distr_pos(0)+7) = electron(k)%tag
           distr_pos(0) = distr_pos(0) + 8
        END DO
...
! prepare maximal values of index for each species
     max_distr_pos(0) = 8 * N_to_distribute(0)
     DO s = 1, N_spec
...
! electrons
     IF (distr_pos(0).LT.max_distr_pos(0)) THEN
        delta_N = (max_distr_pos(0)-distr_pos(0)+1) / 8
! check and resize if necessary the size of the electron array
        ...
        DO k = N_electrons+1, N_electrons+delta_N
           ...
           electron(k)%tag = INT(rbufer(distr_pos(0)+7))
           distr_pos(0) = distr_pos(0) + 8
        END DO
...
! send extra electrons, if applicable
     IF (N_electrons.GT.avg_N_particles(0)) THEN
        message_size = 8 * (N_electrons - avg_N_particles(0))
        ...
        DO k = avg_N_particles(0)+1, N_electrons
           ...
           rbufer(ibuf+7) = DBLE(electron(k)%tag)
           ibuf = ibuf + 8
        END DO
        N_electrons = avg_N_particles(0)
...
! make sure that particle arrays have size enough to accommodate new electrons
        ...
        END IF
        message_size = 8 * (avg_N_particles(0) - N_electrons)
        ...
        DO k = N_electrons+1, avg_N_particles(0)
           ...
           electron(k)%tag = INT(rbufer(ibuf+7))
           ibuf = ibuf + 8
        END DO
        N_electrons = avg_N_particles(0)
        DEALLOCATE(rbufer, STAT=ALLOC_ERR)
