...
! save my own particles first
           ...
           DO i = 1, current_N
              ...
              rbufer(pos+7) = DBLE(electron_to_send_right(i)%tag)
              pos = pos + 8
           END DO
...
! restore my own particles
           ...
           DO k = 1, current_N
              ...
              electron_to_send_right(k)%tag = INT(rbufer(pos+7))
              pos = pos + 8
           END DO
...
! receive the particles
        DO n = Rank_cluster + N_processes_cluster_right, N_processes_cluster-1, N_processes_cluster_right
           ...
           END DO
           Length_to_receive = 8 * sum_N_part_to_receive
...
! send the number of particles
        ...
        END DO
        Length_to_send = 8 * sum_N_part_to_send
...
! save my own particles first
           ...
           DO i = 1, current_N
              ...
              rbufer(pos+7) = DBLE(electron_to_send_left(i)%tag)
              pos = pos + 8
           END DO
! re-size and re-allocate array
...
! restore my own particles
           ...
           DO k = 1, current_N
              ...
              electron_to_send_left(k)%tag = INT(rbufer(pos+7))
              pos = pos + 8
           END DO
           DEALLOCATE(rbufer, STAT = ALLOC_ERR)
        END IF
...
! electrons first
           ...
           DO i = 1, N_part_to_receive(0)
              ...
              IF ((y.GE.c_Y_area_min).AND.(y.LE.c_Y_area_max)) THEN
                 CALL ADD_ELECTRON_TO_ADD_LIST(x, y, vx, vy, vz, ax, ay, tag)
              ELSE IF (y.LT.c_Y_area_min) THEN
                 IF (Rank_of_master_below.GE.0) THEN
                    CALL ADD_ELECTRON_TO_SEND_BELOW(x, y, vx, vy, vz, ax, ay, tag)              
                 ELSE
              ...
              ELSE IF (y.GT.c_Y_area_max) THEN
                 IF (Rank_of_master_above.GE.0) THEN
                    CALL ADD_ELECTRON_TO_SEND_ABOVE(x, y, vx, vy, vz, ax, ay, tag)              
                 ELSE
...
! save my own particles first
           ...
           DO i = 1, current_N
              ...
              rbufer(pos+7) = DBLE(electron_to_send_above(i)%tag)
              pos = pos + 8
           END DO
! re-size and re-allocate array
...
! restore my own particles
           ...
           DO k = 1, current_N
              ...
              electron_to_send_above(k)%tag = INT(rbufer(pos+7))
              pos = pos + 8
           END DO
           DEALLOCATE(rbufer, STAT = ALLOC_ERR)
        END IF
...
! place electrons first
           ...
           DO i = 1, N_e_to_send_above
              ...
              rbufer(pos+5) = electron_to_send_above(i)%AX !
              rbufer(pos+6) = electron_to_send_above(i)%AY !
!### was here              rbufer(pos+7) = electron_to_send_above(i)%VZ ! expected AZ but AZ is not used anyway
              rbufer(pos+7) = DBLE(electron_to_send_above(i)%tag)
              pos = pos + 8
           END DO
! place ions
...
! save my own particles first
           ...
           DO i = 1, current_N
              ...
              rbufer(pos+7) = DBLE(electron_to_send_below(i)%tag)
              pos = pos + 8
           END DO
! re-size and re-allocate array
...
! restore my own particles
           ...
           DO k = 1, current_N
              ...
              electron_to_send_below(k)%tag = INT(rbufer(pos+7))
              pos = pos + 8
           END DO
           DEALLOCATE(rbufer, STAT = ALLOC_ERR)
        END IF
...
! ## 14 ## receive particles from below
        IF (Length_to_receive.GT.0) THEN
           ...
           DO i = 1, N_part_to_receive(0)
              ...
              ELSE IF (x.LT.c_X_area_min) THEN
                 IF (Rank_of_master_left.GE.0) THEN
                    CALL ADD_ELECTRON_TO_SEND_LEFT(x, y, vx, vy, vz, ax, ay, tag)              
                 ELSE
              ...
              ELSE IF (x.GT.c_X_area_max) THEN
                 IF (Rank_of_master_right.GE.0) THEN
                    CALL ADD_ELECTRON_TO_SEND_RIGHT(x, y, vx, vy, vz, ax, ay, tag)              
                 ELSE
