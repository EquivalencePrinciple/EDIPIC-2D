...
!  USE ParallelOperationValues
  ...
  REAL(8) ax_new, ay_new  ! acceleration, can be set to 0 or based on local fields
...
! make sure that the energy of the emitted electron does not exceed the present leftover of the primary electron energy
  ...
  CALL ADD_ELECTRON_TO_ADD_LIST(x_new, y_new, vx_new, vy_new, vz_new, ax_new, ay_new, tag_new)    ! the particle gets tag equal to nwo - the number of the whole object that emitted it
...
! collision with the wall below
  ...
  CALL ADD_ION_TO_ADD_LIST(s, x_new, y_new, vx_new, vy_new, vz_new, ax, ay, tag_new)    ! the particle gets tag equal to the number of the whole object that reflected it
